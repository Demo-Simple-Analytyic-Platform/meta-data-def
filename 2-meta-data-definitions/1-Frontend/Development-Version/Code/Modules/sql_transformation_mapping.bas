Attribute VB_Name = "sql_transformation_mapping"
Option Compare Database
Option Explicit

Public Type typ_transformation_mapping
    id_attribute              As String
    id_transformation_mapping As String
    tx_transformation_mapping As String
    is_in_group_by            As Boolean
End Type

Public Sub parse_transformation_mapping(ip_id_model As String, ip_id_dataset As String, ip_id_transformation_part As String, ip_tx_transformation_part As String, Optional is_debugging As Boolean = False, Optional is_testing As Boolean = False)
    '
    ...
    Dim sql              As String
    Dim map              As typ_transformation_mapping
    '
    If is_debugging Then Debug.Print vbNewLine & String(40, "=") & vbNewLine & " Mappings:" & vbNewLine
    '
    Set db = CurrentDb
    '
    ...
    If ni_pos_begin > 0 Then
        ...
        If is_debugging Then Debug.Print "SELECT clause: " & tx_sql_statement
        If is_debugging Then Debug.Print String(40, "-")
    End If
    '
    ...
    If InStr(1, ip_tx_transformation_part, "GROUP BY", vbTextCompare) > 0 Then
        ...
        If is_debugging Then Debug.Print "GROUP BY clause: " & tx_sql_group_by
        '
    End If
    '
    If (1 = 1) Then ' /* "Temp"-table: at -> list expacted Attributes(s) */
        sql = "DELETE * FROM tmp_transformation_mapping_at"
        DoCmd.SetWarnings False: DoCmd.RunSQL sql: DoCmd.SetWarnings True
        
        sql = "INSERT INTO tmp_transformation_mapping_at (id_attribute, ni_ordering, nm_target_column, tx_to_be_searched_1, tx_to_be_searched_2, tx_sql_statement) " _
        ...
        If is_debugging Then Debug.Print "SQL: " & vbNewLine & sql
        DoCmd.SetWarnings False: DoCmd.RunSQL sql: DoCmd.SetWarnings True
        
        If is_debugging Then
            sql = "SELECT * FROM tmp_transformation_mapping_at ORDER BY ni_ordering ASC"
    ...
    If (1 = 1) Then ' /* "Temp"-table: md -> Find de postions of "alias" of expected "target"-columns */
        sql = "DELETE * FROM tmp_transformation_mapping_md"
        DoCmd.SetWarnings False: DoCmd.RunSQL sql: DoCmd.SetWarnings True
        
        sql = "INSERT INTO tmp_transformation_mapping_md (id_attribute, ni_ordering, nm_target_column, ni_position_till, ni_to_be_searched_length, tx_to_be_searched, dummy) " _
            ...
            & "       1 As ni_dummy " _
            & "FROM tmp_transformation_mapping_at AS att"
        If is_debugging Then Debug.Print "SQL: " & vbNewLine & sql
        DoCmd.SetWarnings False: DoCmd.RunSQL sql: DoCmd.SetWarnings True

        If is_debugging Then
            sql = "SELECT * FROM tmp_transformation_mapping_md ORDER BY ni_ordering ASC"
            Set rst = db.OpenRecordset(sql): With rst: Do Until .EOF
    ...
    If (1 = 1) Then ' /* "Temp"-table: ls -> Last "ni_ordering" */
        sql = "DELETE * FROM tmp_transformation_mapping_ls"
        DoCmd.SetWarnings False: DoCmd.RunSQL sql: DoCmd.SetWarnings True
        
        sql = "INSERT INTO tmp_transformation_mapping_ls (ni_ordering) " _
            & "SELECT MAX(md.ni_ordering) AS ni_ordering " _
            & "FROM tmp_transformation_mapping_md AS md"
        If is_debugging Then Debug.Print "SQL: " & vbNewLine & sql
        DoCmd.SetWarnings False: DoCmd.RunSQL sql: DoCmd.SetWarnings True

        If is_debugging Then
            sql = "SELECT * FROM tmp_transformation_mapping_ls ORDER BY ni_ordering ASC"
            Set rst = db.OpenRecordset(sql): With rst: Do Until .EOF
    ...
    If (1 = 1) Then ' /* "Temp"-table: be -> Begin and End. */
        sql = "DELETE * FROM tmp_transformation_mapping_be"
        DoCmd.SetWarnings False: DoCmd.RunSQL sql: DoCmd.SetWarnings True
        
        sql = "INSERT INTO tmp_transformation_mapping_be (id_attribute, ni_ordering, nm_target_column, ni_position_from, ni_position_till) " _
            & "SELECT md.id_attribute, md.ni_ordering, md.nm_target_column, " _
            & "       Iif(" _
            & "         Nz((SELECT MAX(lag.ni_position_till)         FROM tmp_transformation_mapping_md AS lag WHERE lag.ni_position_till < md.ni_position_till), 1) = 1, 1," _
            & "         Nz((SELECT MAX(lag.ni_position_till)         FROM tmp_transformation_mapping_md AS lag WHERE lag.ni_position_till < md.ni_position_till), 1) + " _
            & "         Nz((SELECT MAX(lag.ni_to_be_searched_length) FROM tmp_transformation_mapping_md AS lag WHERE lag.ni_position_till < md.ni_position_till), 1) + 2 " _
        ...
        If is_debugging Then
            sql = "SELECT * FROM tmp_transformation_mapping_be ORDER BY ni_ordering ASC"
            Set rst = db.OpenRecordset(sql): With rst: Do Until .EOF
    ...
    If (1 = 1) Then ' /* "Temp"-table: be -> Mapping. */
        sql = "DELETE * FROM dta_transformation_mapping " _
            & "WHERE id_model               = '" & ip_id_model & "' " _
            & "AND   id_transformation_part = '" & ip_id_transformation_part & "'"
        If (is_testing = False) Then DoCmd.SetWarnings False: DoCmd.RunSQL sql: DoCmd.SetWarnings True
        '
        ' Open Recordset Transformation Mapping to add records
        Set tmp = db.OpenRecordset("SELECT * FROM dta_transformation_mapping WHERE 1=2")
        '
        ' Loop thought the Begin-End dataset.
        Set rst = db.OpenRecordset("SELECT * FROM tmp_transformation_mapping_be ORDER BY ni_ordering ASC"): With rst: Do Until .EOF
            '
            ' Population Transformation Map record
            map.id_attribute = !id_attribute
            map.id_transformation_mapping = CreateMD5("|" & ip_id_model & "|" & !id_attribute & "|" & ip_id_transformation_part & "|")
            map.tx_transformation_mapping = TRIM(IIf(!ni_position_from = 0, "", Mid(tx_sql_statement, !ni_position_from, (!ni_position_till - !ni_position_from))))
            map.is_in_group_by = IIf(!ni_position_till = 0, 0, IIf(InStr(1, tx_sql_group_by, map.tx_transformation_mapping, vbTextCompare), 1, 0))
            '
            ' if 1st character is "," then remove that and trim again
            If (Left(map.tx_transformation_mapping, 1) = ",") Then
                map.tx_transformation_mapping = TRIM(Mid(map.tx_transformation_mapping, 2, Len(map.tx_transformation_mapping)))
            End If
            
            '
            ' Add record to dta_transformation_mapping.
            tmp.AddNew
            ...
            tmp.fields("id_transformation_mapping") = map.id_transformation_mapping
            tmp.fields("tx_transformation_mapping") = map.tx_transformation_mapping
            tmp.fields("is_in_group_by") = map.is_in_group_by
            If is_testing = False Then tmp.Update
            '
            If is_debugging Then
                Debug.Print "--- be " & String(40, "-")
                ...
                Debug.Print "id_transformation_mapping : '" & map.id_transformation_mapping & "'"
                Debug.Print "tx_transformation_mapping : '" & map.tx_transformation_mapping & "'"
